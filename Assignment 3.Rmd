...
VERTEX1 <- select(VERTEX, id, gender.from, major.from)

#rename variables
names(VERTEX1) <- c("id", "gender", "major")

#Remove all the repeats so that we just have a list of each student and their characteristics
VERTEX1 <- unique(VERTEX1)
...
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX1$gender)
...
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX1$gender, edge.width=EDGE$count)
...
```{r}
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX1$gender, edge.width=EDGE$count, edge.arrow.size=0.5, vertex.size=15, vertex.label.cex=0.8) 
```
* The vertices are colored according to major
```{r}
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX1$major, edge.width=EDGE$count, edge.arrow.size=0.5, vertex.size=15, vertex.label.cex=0.8) 
```

* The vertices are sized according to the number of comments they have received
```{r}
VERTEX$gender.to <- ifelse(is.na(VERTEX$gender.to) == TRUE, as.factor(as.character(VERTEX$gender.from)), as.factor(as.character(VERTEX$gender.to)))

VERTEX$major.to <- ifelse(is.na(VERTEX$major.to) == TRUE, as.factor(as.character(VERTEX$major.from)), as.factor(as.character(VERTEX$major.to)))

VERTEX2 <- select(VERTEX, id, gender.to, major.to)
names(VERTEX2) <- c("id", "gender", "major")
VERTEX2 <- unique(VERTEX2)
library(igraph)
g1 <- graph.data.frame(EDGE, directed=TRUE, vertices=VERTEX2)
plot(g1,layout=layout.fruchterman.reingold)
plot(g1,layout=layout.fruchterman.reingold, vertex.color=VERTEX2$major)
plot(g1,layout=layout.fruchterman.reingold, vertex.color=VERTEX2$major, vertex.size=EDGE$count, edge.arrow.size=0.3, vertex.label.dist=2)
```
...
library(stringr)
library(igraph)
DF1 <- read.csv("hudk4050-classes.csv", header = TRUE, stringsAsFactors = FALSE)
colnames(DF1) <- DF1[1,]
DF1<- DF1[c(-2),]
DF1<- DF1[c(-1),]
DF1 <- select(DF1, 1:8)
...
DF1$name <- str_to_title(DF1$name)
DF1 <- DF1 %>% mutate_at(2:7, list(toupper))
DF1 <- DF1 %>% mutate_at(2:7, str_replace_all, " ", "")

DF2 <- DF1 %>% gather(label, class, 2:7, na.rm = TRUE, convert = FALSE) %>% select(name, class)
DF2$count <- 1
DF2 <- filter(DF2, class != "")
DF2 <- unique(DF2)
DF2 <- spread(DF2, class, count)
rownames(DF2) <- DF2$name
DF2 <- select(DF2, -name, -HUDK4050)
DF2[is.na(DF2)] <- 0
DF3 <- as.matrix(DF2)
DF3 <- DF3 %*% t(DF3)
g <- graph.adjacency(DF3, mode = "undirected", diag = FALSE)
plot(g, layout = layout.fruchterman.reingold, vertex.size = 4, vertex.label.cex = 0.8, vertex.label.color = "purple",
     vertex.color = "grey")
```
...
# Guoliang Xu, Hangshi Jin, Jiaao  Qi, Jiacong Zhu, Jiahao Shen, Wenqi Gao, Xiyun  Zhang, Yingxin Xie
...
plot(g, layout = layout.fruchterman.reingold, vertex.size = 4, vertex.label.cex = 0.8, vertex.label.color = "purple",
     vertex.color = "pink")
# There is no salient correspondence between interest and clusters in the network. The same interest areas (such as prediction modeling and social network analysis are included in all the clusters.)
```
